...
def minimisationfirst(mat,v,c):
        ...
        for i in range(c):
            base.append(v + i + 1)
        x = len(mat[0])
        while check_simplex_w(mat, height, width) == 1 :
            if x < len(mat[0]) :
                for row in mat:
                    row.pop()
                height = len(mat)
            ...
            pos = ratio_simplex_w(mat, height, width, c, v)
            for i in range(c):
        ...
        tab.append(f"This is the value of W = {mat[height-1][width-2]} ")
